...
{
    public partial class gouwu1 : System.Web.UI.Page
    {
        ...
        protected void Luru_Click(object sender, EventArgs e)
        {
            ...
            string strGoodsCode = this.txtGoodsCode.Text.Trim();
            string strConut = this.txtSum.Text.Trim();

            int count = 1;
            if (strConut != "")
            {
                if (!int.TryParse(strConut, out count))
                {
                    Common.ShowMsg("商品数量转换失败！");
                    return;
                }
                if (count <= 0)
                {
                    Common.ShowMsg("商品数量必须大于零！");

                    return;
                }
            }
            if (dtGoods.Select("GoodsCode = '" + strGoodsCode.ToUpper() + "'").Length > 0)
            {
                for (int i = 0; i < dtGoods.Rows.Count; i++)
                {
                    if (dtGoods.Rows[i]["GoodsCode"].ToString().Trim().ToUpper() == strGoodsCode.ToUpper())
                    {
                        dtGoods.Rows[i]["Count"] = Convert.ToInt32(dtGoods.Rows[i]["Count"].ToString().Trim()) + count;
                        dtGoods.Rows[i]["Money"] = Convert.ToInt32(dtGoods.Rows[i]["Count"].ToString().Trim()) * Convert.ToDouble(dtGoods.Rows[i]["Price"].ToString().Trim());
                    }
                }
            }
            else
            {
                string strSql = "select GoodsName,Code,Price from Goods where Code = '" + strGoodsCode + "'";
                DBManager db = DBManager.Instance();
                DataTable dt = db.GetDataTable(strSql);

                if (dt.Rows.Count <= 0)
                {
                    Common.ShowMsg("不存在此商品！");
                    return;
                }

                double price = Convert.ToDouble(dt.Rows[0]["Price"].ToString().Trim());
                double money = count * price;

                DataRow dr = dtGoods.NewRow();
                dr["idno"] = "";
                dr["GoodsCode"] = dt.Rows[0]["Code"].ToString().Trim();
                dr["GoodsName"] = dt.Rows[0]["GoodsName"].ToString().Trim();
                dr["Price"] = price.ToString("0.00");
                dr["count"] = count;
                dr["money"] = money.ToString("0.00");
